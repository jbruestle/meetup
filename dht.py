...
class DhtRpc(object):
    def __init__(self, dht, asm, uport):
        self.dht = dht
        self.asm = asm
        self.uport = uport
        self.uport.add_protocol('d', self.handle_read)
        self.tid = 0
        self.pending = {}
        self.handlers = {}
    ...
    def send_request(self, addr, rtype, args, on_succ, on_fail):
        ...
        self.uport.sendto(raw, addr)

    def add_handler(self, name, callback):
        self.handlers[name] = callback

    def handle_read(self, raw, addr):
        try:
...
class Dht(object):
    def __init__(self, asm, mid, uport):
        self.asm = asm
        self.rpc = DhtRpc(self, self.asm, uport)
        self.mid = mid
        self.locations = {}
